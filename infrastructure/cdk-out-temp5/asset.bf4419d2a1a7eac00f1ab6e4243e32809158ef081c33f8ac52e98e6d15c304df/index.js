...
async function generateScriptFromProject(requestBody) {
    ...
    try {
        ...
        const totalDuration = scriptData.totalDuration || scriptData.estimatedDuration || optimalDuration;
        const sceneCount = scriptData.scenes?.length || 0;
        
        // Generate enhanced scenes with AI-powered visual requirements
        const enhancedScenes = await Promise.all((scriptData.scenes || []).map(async (scene) => ({
            ...
            // Enhanced Media Requirements for Media Curator
            mediaRequirements: await generateEnhancedVisualRequirements(scene, topicContext)
        })));

        const sceneContext = {
            // Required fields for context validation (matching working implementation)
            scenes: enhancedScenes,
            
            // Required context fields (matching working implementation from summary)
            ...
            videoStructure: {
                ...
                totalDuration: totalDuration,
                averageSceneLength: scriptData.sceneStructure?.averageSceneLength || `${Math.round(totalDuration / (sceneCount || 1))} seconds`,
...
/**
 * Generate enhanced visual requirements for Media Curator using AI
 * This provides specific, actionable visual guidance for each scene
 */
async function generateEnhancedVisualRequirements(scene, topicContext) {
    try {
        const prompt = `You are an expert video production visual director. Generate specific, actionable visual requirements for this video scene to help a media curator find the perfect stock footage and images.

SCENE CONTEXT:
- Scene ${scene.sceneNumber}: ${scene.title || 'Untitled'}
- Duration: ${scene.duration}s
- Purpose: ${scene.purpose || 'content'}
- Script: "${scene.script?.substring(0, 300) || 'No script'}..."
- Tone: ${scene.tone || 'informative'}

VIDEO CONTEXT:
- Overall Topic: ${topicContext.mainTopic}
- Target Audience: ${topicContext.targetAudience || 'general'}
- Content Type: Travel/Educational

TASK: Generate specific visual requirements that will help find the most relevant stock media. Be very specific about:
1. Exact locations, objects, or subjects to search for
2. Specific camera angles and shot types
3. Lighting and mood preferences
4. People and activities (if relevant)
5. Architectural or natural elements (if relevant)

Respond with a JSON object in this exact format:
{
  "specificLocations": ["location1", "location2"],
  "visualElements": ["element1", "element2", "element3"],
  "shotTypes": ["wide shot", "close-up", "detail shot"],
  "searchKeywords": ["keyword1", "keyword2", "keyword3", "keyword4", "keyword5"],
  "mood": "descriptive mood",
  "timeOfDay": "preferred time",
  "peopleActivities": ["activity1", "activity2"],
  "architecturalElements": ["element1", "element2"],
  "primaryFocus": "main subject",
  "secondaryElements": ["supporting element1", "supporting element2"],
  ...
  "qualityPreferences": {
    ...
    "preferredOrientation": "landscape"
        ...
        const response = await bedrockClient.send(command);
        const result = JSON.parse(new TextDecoder().decode(response.body));
        const enhancedRequirements = JSON.parse(result.content[0].text);

        // Add fallback and legacy compatibility
        return {
            ...enhancedRequirements,
            // Legacy compatibility
            primaryVisualType: enhancedRequirements.primaryFocus || scene.mediaNeeds?.[0] || 'presenter',
            supportingVisuals: enhancedRequirements.secondaryElements || scene.mediaNeeds?.slice(1) || [],
            visualComplexity: scene.visualStyle === 'dynamic_engaging' ? 'high' : 'medium',
            emotionalContext: enhancedRequirements.mood || scene.tone || 'informative',
            searchKeywords: enhancedRequirements.searchKeywords || extractSceneKeywords(scene.script, scene.keyPoints || [])
        };
        ...
        return {
            ...
            specificLocations: [topicContext.mainTopic],
            visualElements: ['generic content'],
            shotTypes: ['medium shot'],
            mood: scene.tone || 'informative',
            timeOfDay: 'any',
            peopleActivities: [],
            architecturalElements: [],
            primaryFocus: scene.mediaNeeds?.[0] || 'content',
            secondaryElements: [],
            avoidElements: [],
            qualityPreferences: {
                imageCount: 3,
