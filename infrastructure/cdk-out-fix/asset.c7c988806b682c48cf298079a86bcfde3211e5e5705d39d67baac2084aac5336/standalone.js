  ...
  "visualRequirements": ["Visual 1", "Visual 2", "Visual 3"]
    ...
    if (bucketName) {
      try {
        ...
        const scriptKey = `videos/${finalProjectId}/02-script/script.json`;
        await s3Client.send(new PutObjectCommand({
          Bucket: bucketName,
          Key: scriptKey,
          Body: JSON.stringify(scriptData, null, 2),
          ContentType: 'application/json'
        }));
                
        console.log(`✅ Stored script to S3: ${scriptKey}`);
      } catch (s3Error) {
        console.error('❌ Failed to store script to S3:', s3Error);
      }
    }
