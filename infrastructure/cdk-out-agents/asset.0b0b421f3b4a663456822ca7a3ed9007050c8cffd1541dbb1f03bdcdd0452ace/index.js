  ...
  switch (httpMethod) {
    ...
    case 'POST':
      if (path === '/async/start-pipeline') {
        return await startAsyncPipeline(requestBody);
      } else if (path === '/async/process-job') {
        // Internal processing endpoint (invoked by other Lambdas)
        return await processAsyncJob(requestBody);
      }
      break;
  }
 ...
 */
async function startAsyncPipeline(requestBody) {
 ...
 */
async function processAsyncJob(requestBody) {
