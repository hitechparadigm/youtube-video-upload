...
const handler = async (event, context) => {
  ...
  if (httpMethod === 'POST' && path === '/video/assemble') {
    ...
    try {
      ...
      // Upload video metadata to S3
      const metadataKey = `videos/${projectId}/05-video/video-metadata.json`;
      await uploadToS3(metadataKey, JSON.stringify(videoMetadata, null, 2), 'application/json');
      console.log(`✅ Video metadata uploaded: ${metadataKey}`);

      // Upload assembly script to S3
      const scriptKey = `videos/${projectId}/05-video/assembly-script.json`;
      await uploadToS3(scriptKey, JSON.stringify(assemblyScript, null, 2), 'application/json');
      console.log(`✅ Assembly script uploaded: ${scriptKey}`);
      ...
      const infoKey = `videos/${projectId}/05-video/video-info.json`;
      await uploadToS3(infoKey, JSON.stringify(videoInfo, null, 2), 'application/json');
      console.log(`✅ Video info uploaded: ${infoKey}`);
