...
async function generateAudioFromProject(requestBody, context) {
    return await monitorPerformance(async () => {
        ...
        const basicAudioContext = {
            ...
            scenesPlanned: sceneContext.scenes && sceneContext.scenes.length || 0,
        ...
        for (let i = 0; i < (sceneContext.scenes || []).length; i++) {
            ...
            const words = (scene.content && scene.content.script && scene.content.script.split(' ')) || [];
        ...
        const audioContext = {
            ...
            synchronizationData: {
                sceneBreakpoints: sceneContext.scenes && sceneContext.scenes.map(scene => ({
                    sceneNumber: scene.sceneNumber,
...
function calculateSceneAwarePacing(scene, mediaContext) {
    ...
    const sceneMedia = mediaContext && mediaContext.sceneMediaMapping && mediaContext.sceneMediaMapping.find(m => m.sceneNumber === scene.sceneNumber);
    const visualChanges = sceneMedia && sceneMedia.mediaSequence && sceneMedia.mediaSequence.length || 3;
...
async function generateSceneAudio(scene, voice, pacingConfig, projectId) {
    const script = (scene.content && scene.content.script) || scene.script || '';
...
async function createMasterAudio(audioSegments, masterAudioId) {
    ...
    const masterAudioKey = `videos/${audioSegments[0] && audioSegments[0].audioKey && audioSegments[0].audioKey.split('/')[1] || 'unknown'}/04-audio/master-narration.mp3`;
    ...
    const masterAudioInfo = {
        ...
        projectId: audioSegments[0] && audioSegments[0].audioKey && audioSegments[0].audioKey.split('/')[1] || 'unknown',
...
async function generateAudio(requestBody, context) {
    return await monitorPerformance(async () => {
        ...
        textLength: requestBody.text && requestBody.text.length || 0
    });
}
